...
def content_columns_alignment(num_small_columns: int, num_large_columns: int) -> str:
    """
...
def double_header_table(first_columns_names: List[str],
    ...
    table = Tabular(f"{first_columns_alignment(num_first_columns)}{content_columns_alignment(num_small_columns, num_large_columns)}")
    ...
    table.add_row(
        (
            ...
            *[MultiColumn(size=large_column_size,
                          align="c||" if i != num_large_columns - 1 else "c|",
    ...
    table.add_row(["" for _ in range(num_first_columns)] + small_column_names)
    table.add_hline()
    ...
    for i in range(content_height):
        ...
        table.add_row((*first_columns_row_values, *content_row_values))
        table.add_hline()
